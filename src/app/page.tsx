...
export default function page() {
  ...
  return (
    <>
      <div className="fixed top-0 left-0 h-screen w-screen flex items-center justify-center z-50 backdrop-blur-xl">
        <div className="p-5 rounded-lg bg-white flex flex-col items-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-blue-500"></div>
          <p className="mt-3 font-semibold">Processing...</p>
        </div>
      </div>
      <div className={`flex flex-col ${loading ? "h-screen w-screen overflow-hidden" : ""}`}>
        <div>
          <p className=" text-1xl text-center font-bold text-neutral-950 bg-neutral-50">{appState.message}</p>
        </div>
        <div className="flex flex-col items-center  w-full gap-10 h-svh bg-neutral-50">
          <div className="flex flex-col gap-4 justify-center items-center">
            <h1 className="font-extrabold text-neutral-950 lg:text-7xl  md:text-6xl text-4xl text-center mt-10">Dog and Cat Classifier</h1>

            <p className="font-extrabold text-center">Classifications: {dbData ? dbData.classifications : 0}</p>
          </div>
          <div className="flex flex-col items-center justify-center w-full h-fit gap-6">
            <div
              className={`flex flex-col items-center justify-center w-full max-w-3xl gap-4 transition duration-300 ${
                results.result ? "scale-100 pointer-events-auto" : "scale-0 pointer-events-none h-0"
              }`}>
              ...
              <div className="max-w-full flex flex-wrap justify-center items-center">
                <p className="max-w-8/12 text-center text-base font-medium text-neutral-950 text-wrap">Dark blue: Less important for the classification, Red: More important for the classification</p>
              </div>
            ...
            <div
              className={`flex flex-col items-center justify-center w-full gap-6 transition duration-100 ease-in-out ${
                results.result ? "scale-0 pointer-events-none h-0 opacity-0" : "opacity-100 scale-100 pointer-events-auto h-fit"
              }`}>
              <label
                ...
                <div
                  className={`flex flex-col gap-2 items-center justify-center transition duration-300 ease-in-out ${
                    preview ? "scale-100 pointer-events-auto h-full" : "scale-0 pointer-events-none h-0"
                  }`}>
                  <img src={preview ?? undefined} alt="Preview" className="rounded-md opacity-80 h-9/12" />
                  <button
                    type="button"
                    className={`px-8 py-1 text-sm font-medium transition duration-100 ease-in-out bg-red-500 cursor-pointer rounded-xl text-neutral-50 hover:bg-red-700 ${
                      preview ? "scale-100 pointer-events-auto" : "scale-0 pointer-events-none h-0 w-0"
                    }`}
                    onClick={Reset}>
                    Change image
                  </button>
              ...
              <p className="text-neutral-950 text-center">Supported file types: We don't know (Jpeg?, PNG?)</p>
              ...
              <div className="flex flex-row justify-center gap-6 items-center px-5">
                ...
                <div className="group relative inline-block">
                  <MainButton text="RF" onClick={handleUpload} className="!important hover:bg-green-500!" value="RF" />
                  <span className="absolute invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs bg-gray-800 text-white rounded whitespace-nowrap">
                    Random Forest
                  </span>
                </div>
